...
const Login = () => {
	...
	const handleClick = async () => {
		const res = await login(formData, setLoading, setError, setLoggedIn)
		if (loggedIn) {
			setFormData({
				email: "",
				password: ""
			...
			<form onSubmit={e => e.preventDefault()} className='card bg-base-200 w-72  md:w-90 mx-auto shadow-gray-400 shadow-xl'>
				<div className='card-body'>
					...
					<div className='card-actions justify-end mt-4'>
						<button type="submit" className={`btn btn-primary`} disabled={loading} onClick={handleClick}>{loading ? <Loading /> : 'Login'}</button>
					</div>
				</div>
			</form>
